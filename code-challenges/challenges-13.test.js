...
// Solution code here...
  ...
  arr.forEach((str,index)=>{
    return arr[index].length>arr[ans].length?ans=index:null;
...
const findHappiness = (arr) => {
  // Solution code here...
  let ans=[];
  arr.forEach(str=>{if(str.includes(':)')){ans.push(str);}});
  return ans;
};
...
const onlyOddChars = (str) => {
  ...
  let ans='';
  for (let i = 0; i < str.length; i++) { 
    str.indexOf(str.charAt(i))%2?ans+=str[i]:null;
  }
  return ans;
};
