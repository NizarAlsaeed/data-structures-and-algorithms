...
const updateNumbers = (obj) => {
  ...
  for(let i=0;i<Object.keys(obj).length; i++){
    ans.push(`${Object.keys(obj)[i]}: ${Object.values(obj)[i]}`);
  };
  return ans;
};
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  sortByChildren(arr).forEach(obj=>houses.push(obj.house));
...
const hasChildrenValues = (arr, character) => {
  ...
  if(obj.name===character){
    hasChilds = obj.children.length>1?true:false
  } 
  });
