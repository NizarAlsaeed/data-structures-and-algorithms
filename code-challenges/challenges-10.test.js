...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((accumulator,value)=>accumulator.concat(value),[]).filter(x=>x===target).length 
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.map(subArr=>subArr.reduce((t,v)=>t+=v)).reduce((t,v)=>t+=v);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(subArr=>subArr.filter(item=>item%5===0).map(item=>Math.pow(2,item)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(obj=>(obj.gender==='male' || obj.gender==='female')).map(obj=>{
...
let findShortest = (data) => {
  ...
  return data.map(obj=>{
    return Object.values(obj)[0]
  }).forEach(item=>item.length<ans?ans=item.length:false);
};
